...
#   bash <(curl -Ls https://raw.githubusercontent.com/Astrenix/version-controller/main/tovanix-proxy-node/install.sh) --force
#   bash <(curl -Ls https://raw.githubusercontent.com/Astrenix/version-controller/main/tovanix-proxy-node/install.sh) --help
#
# 默认:
...
# 产物从【公开的】version-controller 取,源码仓 Astrenix/tovanix-proxy-node 是私有的。
#
# 🩸 私有仓的 release 资产【不带 token 下载不了】—— 装机脚本必须零认证可跑,
# 否则每台节点都要塞一份 GitHub token,那既是运维负担也是凭据扩散。
# 所以走「私密源码仓 + 公开产物仓」:CI 在私有仓编译,把产物发到 version-controller。
GH_OWNER="${GH_OWNER:-Astrenix}"
GH_REPO="${GH_REPO:-version-controller}"
# PROJECT 同时是 release tag 前缀与 tarball 前缀。version-controller 里放着多个
# 项目的产物,tag 形如 tovanix-proxy-node-v1.0.0 —— 取版本时【必须】按这个前缀
# 过滤,否则会把别的项目(linkflow 等)的最新版当成自己的。
PROJECT="${PROJECT:-tovanix-proxy-node}"
INSTALL_DIR="${INSTALL_DIR:-/usr/local/${CMD_NAME}}"
PKG_PREFIX="${PKG_PREFIX:-${PROJECT}}"   # tarball 解压顶层目录名,与 release 包结构一致
...
resolve_version() {
    ...
    # version-controller 是【多项目共用】的产物仓,所以不能用 /releases/latest,
    # 也不能用 /releases?per_page=1 —— 前者忽略 prerelease,后者返回的很可能是
    # 别的项目刚发的版本。必须拉一页回来,按 "${PROJECT}-v" 前缀自己挑。
    #
    # 先整段读完再 grep:grep -m1 早退会让 curl 收到 SIGPIPE,
    ...
    if [[ -z "$v" ]]; then
        die "无法获取 ${PROJECT} 的最新版本号(GitHub API 限流?或 ${GH_OWNER}/${GH_REPO} 尚无该项目的 release)"
    fi
    echo "$v"
}

# release_tag 把纯版本号拼成 version-controller 里的完整 tag。
# 项目仓的 tag 是 v1.2.3,产物仓的是 tovanix-proxy-node-v1.2.3 —— 两者有意不同,
# 因为产物仓里住着多个项目,不加前缀会撞车。
release_tag() {
    local v="$1"
    [[ "$v" == ${PROJECT}-* ]] && { echo "$v"; return; }   # 已经带前缀就原样用
    echo "${PROJECT}-${v}"
}
...
download_release() {
    ...
    local version="$1"
    local tag; tag="$(release_tag "${version}")"
    local pkg="${PKG_PREFIX}-linux-${ARCH}.tar.gz"
    local url="https://github.com/${GH_OWNER}/${GH_REPO}/releases/download/${tag}/${pkg}"
    local sum_url="https://github.com/${GH_OWNER}/${GH_REPO}/releases/download/${tag}/checksums.txt"
    ...
    if ! curl -fSL --connect-timeout 10 -o "${dest}" "${url}" >&2; then
        rm -f "${dest}"
        die "下载失败 — release ${tag} 可能不存在,或 GitHub 网络不通"
    fi
...
show_credentials() {
    ...
    # ── 接入业务主控需要的两样东西 ────────────────────────────────────
    #
    # 🩸 2026-09-16:本程序【没有网页面板】。这里从前打印的是「访问地址 + 首装
    # 明文用户名密码」,让人去浏览器里登录 —— 那个地址现在打开是 404,而那对
    # 凭据除了 sui CLI(本来就得 root 登机器才跑得了)之外没有任何用途。
    #
    # 真正需要交出去的是:API 基址 + token。这两样填进主控的节点记录,节点才算接上。
    # 明文密码不再打印:它会留在 SSH scrollback、terminal 日志与录屏里,
    # 而它换不来任何能力 —— 纯风险,零收益。
    echo
    echo -e "${green}接入业务主控(把下面两项填进节点记录):${plain}"
    echo -e "  ${yellow}API 基址${plain}(api_base_url,不含 /api/v1):"
    "${INSTALL_DIR}/sui" uri 2>/dev/null | sed 's/^/    /' || warn "  无法获取 — 检查服务状态"
    echo
    if [[ -n "${INSTALLER_TOKEN}" ]]; then
        echo -e "  ${yellow}API Token${plain}(★ 只显示这一次,丢了只能重新签发):"
        echo -e "    ${green}${INSTALLER_TOKEN}${plain}"
    else
        echo -e "  ${red}API Token 未生成${plain} —— 这台节点当前没有任何可用入口。"
        echo -e "    手动签发: ${cyan}${INSTALL_DIR}/sui token -add -desc manual${plain}"
    fi

    if [[ -n "${INITIAL_PORT}" || -n "${INITIAL_PATH}" ]]; then
        echo
        echo -e "${yellow}端口与路径前缀已随机化(★ 务必记录):${plain}"
        [[ -n "${INITIAL_PORT}" ]] && echo -e "  端口:     ${green}${INITIAL_PORT}${plain}"
        # 这个前缀不是"面板入口"了 —— API 就挂在它下面(web.go: base_url + "api/v1"),
        # 随机化它等于给 /api/v1 加一层不可枚举的前缀,是实打实的防护不是门脸。
        [[ -n "${INITIAL_PATH}" ]] && echo -e "  路径前缀: ${green}${INITIAL_PATH}${plain}  ${cyan}(API 挂在 <前缀>api/v1 下)${plain}"
        if [[ "${SECURE_ENTRY}" = "1" ]]; then
            echo -e "  ${cyan}已启用安全入口:路径丢了只能 ${CMD_NAME} setting -show 查${plain}"
        fi
    fi

    echo
    echo -e "${green}运维命令:${plain}"
    echo "  systemctl status ${SERVICE_NAME}   服务状态"
    echo "  journalctl -u ${SERVICE_NAME} -f   实时日志"
    echo "  ${INSTALL_DIR}/sui setting -show   查端口 / 路径 / 域名 / SSL"
    echo "  ${INSTALL_DIR}/sui token -add      再签一个 API token"
    echo "  ${CMD_NAME}                        交互菜单"
    echo
    echo -e "${cyan}本程序没有网页界面 —— 浏览器打开任何路径都是 404,这是有意的。${plain}"
    echo
}

# ---------- main ----------

echo -e "${green}${PROJECT} · install${plain}"
...
# API token —— 【无条件生成】。
#
# 🩸 2026-09-16 改动:这里从前是 `if [[ -n REPORT_URL && -n REPORT_KEY ]]`,
# 即只有主控自动装机那条路径才发 token。在还有网页面板的年代这说得通:
# 手动装完的人可以开浏览器登面板。现在面板没了,**token 是使用这个程序的
# 唯一方式** —— 手动装完的节点会是一块砖,装的人拿不到任何入口,
# 而且不会有任何报错告诉他缺了什么。
#
